...
| 項目 | 状況 | 備考 |
|------|------|------|
| AGGRESSIVE パターン | ✅ | 最短距離＋最大ダメージ優先、**接近選定をパスコスト基準に改善**、**到達不可時は武器射程内攻撃位置へのフォールバック接近** |
...
| **CAUTIOUS パターン（後の先）** | **✅** | **新規追加: 敵の脅威圏外から攻撃を狙う、**接近選定をパスコスト基準に改善**、**到達不可時は武器射程内攻撃位置へのフォールバック接近** |
| **SUPPORT パターン（援護）** | **✅** | **新規追加: 味方が攻撃中の敵を優先ターゲット、**接近選定をパスコスト基準に改善**、**到達不可時は味方隣接マスへのフォールバック接近** |
| **FLEE パターン（逃走）** | **✅** | **新規追加: 全敵から最大距離に逃走、攻撃しない** |
| **ユニット作戦設定（UnitTactic）** | **✅** | **新規追加: 部隊編成画面でユニットごとに5種の作戦を設定** |
| **HEAL パターン（回復）** | **✅** | **新規追加: 負傷した味方を優先回復、杖未装備時はAGGRESSIVEにフォールバック、**接近ソートをパスコスト基準に改善**、**到達不可時は杖射程内回復位置へのフォールバック接近** |
...
| 移動可能マス計算 (getMovablePositions) | ✅ | getMovablePositionsWithCost のラッパーに統合済み |
| **移動可能マス計算・コストマップ付き (getMovablePositionsWithCost)** | **✅** | **新規追加: 移動コストをMapで返す** |
| 最短経路計算 (findPath) | ✅ | A*、敵ユニットすり抜け防止対応済み、**バウンディングボックスによる探索範囲制限追加** |
| **経路コスト計算 (getPathCostTo)** | **✅** | **ダイクストラ法による実パスコスト取得（テスト用に残存、AI側はbuildCostMapFromに移行）** |
| **コストマップ構築 (buildCostMapFrom)** | **✅** | **新規追加: 目的地から逆方向ダイクストラで一括コストマップ構築（AI最適化）** |
| **複数起点コストマップ (buildCostMapFromMultiple)** | **✅** | **新規追加: multi-source ダイクストラで複数起点からのコストマップを一括構築（到達不可時のAIフォールバック接近用）** |
| **地形通行可否判定 (isPassableFor)** | **✅** | **新規追加: ユニットの移動タイプで座標の通行可否を判定（AIの攻撃/回復位置フィルタ用）** |
| **探索範囲制限 (BoundingBox)** | **✅** | **新規追加: 全ユニット位置+マージン5タイルで探索範囲を制限** |
| ユニット重なり防止 | ✅ | 味方は通過可・敵は不可（getMovablePositions / findPath 両方で統一） |
| 移動不能地形 (WALL) | ✅ | コスト = -1 |
| **テレポート防止 (BattleScreen)** | **✅** | **新規修正: 経路未発見時のフォールバックを直接移動→待機に変更** |
...
| BGM | ❌ | |
| SE（攻撃・移動・選択） | ❌ | |
| フォント | ✅ | FreeType + NotoSansJP.ttf（ホワイトリスト + assets/data漢字自動収集 + incremental、packerサイズ戦略: <=32は1024 / >32は1536、4サイズ: 24/32/48/64） |
| テクスチャ / スプライト | ❌ | |
| アニメーション | ❌ | |
