...
各ユニットのタイル上方に名前を中央揃えで表示する。戦闘準備画面と同じスタイルで、
戦闘中も常にユニットを識別できるようにする。移動アニメ中は補間位置に追従する。

| 項目 | 値 |
|------|------|
| フォント | `FontManager.getFont(size = 16)` |
| 位置 | タイル上端 + 16px 上、中央揃え |
| 色（PLAYER） | 淡い青 (0.7, 0.85, 1.0) |
| 色（ENEMY） | 淡い赤 (1.0, 0.6, 0.6) |
| 色（ALLY） | 淡い緑 (0.6, 1.0, 0.7) |
...
- 欠字対策として、FontManager の漢字ホワイトリスト + assets/data 漢字自動収集を適用したフォントを優先使用する
...
5. **ユニット名ラベル** — BitmapFont (16px, 陣営カラー淡色, タイル上部中央揃え)
6. **ステータスパネル** — ShapeRenderer (Filled) + BitmapFont
7. **調査パネル** — ShapeRenderer (Filled) + BitmapFont
8. **行動順キュー** — ShapeRenderer (Filled) + BitmapFont
9. **ラウンド情報テキスト** — BitmapFont
