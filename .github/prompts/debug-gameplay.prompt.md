...
| 敵ターンで hasActed がリセットされない | startPhase() の呼び出し漏れ | endPlayerPhase() 修正 |
| ALLYフェイズがスキップされる | advancePhase() 後に条件分岐なし | executeAllyPhase() 追加 |
| 変数名がコンパイルエラー | GameUnit への一括リネーム時の正規表現破壊 | 手動修正 |
